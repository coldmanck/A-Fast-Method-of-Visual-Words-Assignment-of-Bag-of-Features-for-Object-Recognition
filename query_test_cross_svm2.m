...
Max = max(CD2);
M = max(Max);
CD3 = CD2/M; % database patch (patches * 256)
QD3 = QD2/M; % query patch (patches * 256)
CCL = double(CL(:, 1)); % Right label of database (patches * 1)
CQL = double(QL(:, 1)); % Right label of query (used to verify, patches * 1)
...
for i=1:numel(C)
    cv_acc(i) = svmtrain(CCL, CD3, sprintf('-c %f -g %f -v %d', 2^C(i), 2^gamma(i), folds));
end
...
%save(['Results/ANN_', dataSet, '_n', num2str(n_of_extract), '_kd', num2str(params.trees), '.mat'], ID, d, CD2', QD2');
