...
nsq = 4;              % number of subquantizers to be used (m in the paper)
coarsek = 128;        % number of centroids for the coarse quantizer
w = 2;                % number of cell visited per query
...
savRecPath = ['Results/', dataSet, '_n', num2str(n_of_extract), '_pq_before.mat'];
save(savRecPath,'rec');
...
savRecPath = ['Results/', dataSet, '_n', num2str(n_of_extract), '_pq_after.mat'];
save(savRecPath,'rec');
