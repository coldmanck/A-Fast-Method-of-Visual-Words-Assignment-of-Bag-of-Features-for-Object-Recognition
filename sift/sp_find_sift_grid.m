function sift_arr = sp_find_sift_grid(I, grid_x, grid_y, patch_size, sigma_edge, subname, fname)
...
% make directories for the cropped images
dir_path = strcat('cropped_images_cifar-10/', subname, '/', fname);
if ~isdir(dir_path),
    mkdir(dir_path);
end;
...
%     weights = weights ./ repmat(tmp, [1 num_samples]);
    ...
    img_path = strcat(dir_path, '/', sprintf('%05d', i), '.jpg');
    imwrite(I(y_lo:y_hi,x_lo:x_hi), img_path);
