function [beta, t_knns] = sc_pooling(feaSet, B, pyramid, gamma, fpath)
%================================================
% 
% Usage:
...
% fpath = 'data/Caltech101/Airplane/image_0001.mat' (e.g.)
% find the nearest label of each local feature
newPath = strrep(fpath, 'data', 'label_3');
newPath = strrep(newPath, '.mat', '_label.mat');
[token1, remain1] = strtok(newPath, '/');
[token2, remain2] = strtok(remain1, '/');
token3 = strtok(remain2, '/');

label_dir = strcat(token1, '/', token2, '/', token3);
if ~isdir(label_dir),
    mkdir(label_dir);
end;
...
tic
%[IDX, D] = knnsearch(NS, feaSet.feaArr', 'k',10,'distance','euclidean');
[IDX, D] = knnsearch(NS, feaSet.feaArr', 'k', 10,'distance','euclidean');
t_knns = toc;
save(newPath, 'IDX');
