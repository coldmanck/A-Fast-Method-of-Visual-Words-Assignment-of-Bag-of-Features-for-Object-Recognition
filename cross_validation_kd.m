function [image_data, label_data, image_query, label_query, sift_query] = cross_validation(index_dir, dataSet, n_of_extract, kd)
...
%     ext = extract(i).ext;
    ...
    % for j = (n_of_extract+1):n_of_extract+2
        path = strcat(f_path, '/', l_path.cname{ext(j)});
...
save_path = strcat('Results/extract/',dataSet, '_extracted_random_n', sprintf('%d', n_of_extract),'_kd', kd, '.mat');
